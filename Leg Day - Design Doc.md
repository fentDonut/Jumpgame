...
- **Upgrading** (currency-bought gear tiers): raises base jump height/distance, and can unlock perks — double jump, fall-damage resist, air dash.
  ...
  - Each tier costs about three cash-out runs. Since the cache is the only ride down, the loop already ends where the shops are: reach a new island → take the arrival bonus → collect the cache → land at the bottom → spend. Selling tiers on the island that unlocks them would have let players buy mid-climb and skip that trip.
